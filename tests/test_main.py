...
def test_non_int_sim_num(tmp_path):
    """
    Edge test: sim_num must be a non-negative integer.
    ...
    assert "Simulation number must be a non-negative integer." in result.stderr


def test_negative_sim_num(tmp_path):
    """
    Edge test: sim_num must be non-negative.
    """
    config = {
        'data_path': 'src/pyvisdmc/test_data',
        'molecule': 'h5o3',
...
def test_zero_walkers(tmp_path):
    """
    Edge test: walkers must be a positive integer.
    """
    config = {
        'data_path': 'src/pyvisdmc/test_data',
        'molecule': 'h5o3',
    ...
    result = run_main(config_file)
    assert result.returncode != 0
    assert "The number of walkers must be a positive integer." in result.stderr


def test_non_int_walkers(tmp_path):
    """
    Edge test: walkers must be an integer.
    """
    config = {
        'data_path': 'src/pyvisdmc/test_data',
        'molecule': 'h5o3',
...
def test_zero_timesteps(tmp_path):
    """
    Edge test: timesteps must be a positive integer.
    """
    config = {
        'data_path': 'src/pyvisdmc/test_data',
        'molecule': 'h5o3',
    ...
    result = run_main(config_file)
    assert result.returncode != 0
    assert "The number of timesteps must be a positive integer." in result.stderr


def test_non_int_timesteps(tmp_path):
    """
    Edge test: timesteps must be an integer.
    """
    config = {
        'data_path': 'src/pyvisdmc/test_data',
        'molecule': 'h5o3',
...
def test_start_exceeds_timesteps(tmp_path):
    """
    Edge test: start must not exceed timesteps.
    Provide start > timesteps.
    """
    config = {
        'data_path': 'src/pyvisdmc/test_data',
        'molecule': 'h5o3',
    ...
    result = run_main(config_file)
    assert result.returncode != 0
    assert "start (25000) must not exceed timesteps (20000)." in result.stderr


def test_non_int_start(tmp_path):
    """
    Edge test: start must be an integer.
    """
    config = {
        'data_path': 'src/pyvisdmc/test_data',
        'molecule': 'h5o3',
    ...
    result = run_main(config_file)
    assert result.returncode != 0
    assert "start must be an integer." in result.stderr


def test_non_int_stop(tmp_path):
    """
    Edge test: stop must be an integer.
    """
    config = {
        'data_path': 'src/pyvisdmc/test_data',
        'molecule': 'h5o3',
    ...
    result = run_main(config_file)
    assert result.returncode != 0
    assert "stop must be an integer." in result.stderr


def test_plots_not_a_list(tmp_path):
    """
    Edge test: plots must be a list of strings.
    Make sure ValueError is raised when providing a string instead of a list.
    """
    config = {
        'data_path': 'src/pyvisdmc/test_data',
        'molecule': 'h5o3',
    ...
    result = run_main(config_file)
    assert result.returncode != 0
    assert "plots must be a list of strings." in result.stderr


def test_plots_not_strings(tmp_path):
    """
    Edge test: plots must be a list of strings.
    Make sure ValueError is raised when providing a list with a non-string element.
    """
    config = {
        'data_path': 'src/pyvisdmc/test_data',
        'molecule': 'h5o3',
...
def test_no_plots_specified(tmp_path):
    ...
    result = run_main(config_file)
    assert result.returncode == 0, "No plots scenario is considered a success, so check for exit message."
    assert "No plots specified in the configuration." in result.stderr
