...
This module provides a function to generate and save histograms
and/or density plots for multiple bond length distributions from
a molecular Diffusion Monte Carlo (DMC) simulation. It calculates
the expectation value (average) of each bond length and overlays
it on the plot.
...
def plot_dists(molecule, sim_num, analyzer, weights, dists,
               hist=True, line=True, exp=True):
    """
    Generate and save plots of multiple bond length distributions
    from a molecular DMC simulation. The function can plot histograms,
    density plots, and overlay expectation values on the plot.

    Parameters:
    - molecule: The molecule being analyzed (e.g., 'h5o3', 'h2o').
    - sim_num: The simulation number.
    - analyzer: An instance of pyvibdmc's AnalyzeWfn class, used to
                analyze wavefunctions.
    - weights: Weights associated with the molecular geometries.
    - dists: List of pairs of atom indices representing bonds
            (e.g., [[0, 1], [2, 3]]).
    - hist: If True, generate a histogram.
    - line: If True, overlay a KDE (Kernel Density Estimate) line
            on the histogram.
        ...
        if line:
            for i in range(len(dist_vals)):
                ...
                sns.histplot(dist_vals[i], kde=True, bins=50, common_norm=True,
                             label=rf'$\langle$r{dists[i][0]}{dists[i][1]}'
                             rf'$\rangle$ = {exp_vals[i]:.4f} $\AA$')
        else:
            for i in range(len(dist_vals)):
                sns.histplot(dist_vals[i], kde=False, bins=50,
                             common_norm=True,
                             label=rf'$\langle$r{dists[i][0]}{dists[i][1]}'
                             rf'$\rangle$ = {exp_vals[i]:.4f} $\AA$')
    else:
    ...
    plt.savefig(f'{molecule}_sim_{sim_num}_mult_dists.png',
                bbox_inches='tight')
